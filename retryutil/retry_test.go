...
func TestRetryFunc(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			f, count := tt.callsCollector(tt.expectedToFail, tt.failWith)
...
func TestRetryAPICall(t *testing.T) {
	tests := []struct {
		...
		{
			name:                 "Function fail with retriable API error, retry does not helped",
		...
		{
			name:                 "Function fail with retriable API error, retry does helped",
			maxRetryTime:         2 * time.Minute,
			callsCollector:       callsCollector,
			expectedToFail:       3,
			failWith:             status.Error(codes.DeadlineExceeded, "invalid"),
...
func callsCollector(expectedToFail int, failWith error) (func() error, *int) {
	var c int
	return func() error {
		c++
		if expectedToFail <= c {
